...
void basicThreadedMultiply(const Matrix& A, const Matrix& B, Matrix& C) {
  ...
  assert(A.numRows() % num_of_threads == 0);
  ...
  for (size_t thread = 1; thread < num_of_threads; ++thread) {
    threads[thread - 1] = std::thread([&, thread]() {
      for (size_t i = thread * A.numRows() / num_of_threads; i < (thread + 1) * A.numRows() / num_of_threads; ++i) {
        for (size_t j = 0; j < B.numCols(); ++j) {
          for (size_t k = 0; k < A.numCols(); ++k) {
            C(i, j) += A(i, k) * B(k, j);
          }
  ...
  for (size_t i = 0; i < A.numRows() / num_of_threads; ++i) {
    for (size_t j = 0; j < B.numCols(); ++j) {
      for (size_t k = 0; k < A.numCols(); ++k) {
        C(i, j) += A(i, k) * B(k, j);
      }
...
void tiledMultiply2x2(const Matrix& A, const Matrix& B, Matrix& C) {
  for (size_t i = 0; i < A.numRows(); i += 2) {
    for (size_t j = 0; j < B.numCols(); j += 2) {
      for (size_t k = 0; k < A.numCols(); ++k) {
        C(i, j) += A(i, k) * B(k, j);
...
void tiledMultiply2x4(const Matrix& A, const Matrix& B, Matrix& C) {
  for (size_t i = 0; i < A.numRows(); i += 2) {
    for (size_t j = 0; j < B.numCols(); j += 4) {
      for (size_t k = 0; k < A.numCols(); ++k) {
        C(i, j) += A(i, k) * B(k, j);
        C(i, j + 1) += A(i, k) * B(k, j + 1);
...
void tiledMultiply4x2(const Matrix& A, const Matrix& B, Matrix& C) {
  for (size_t i = 0; i < A.numRows(); i += 4) {
    for (size_t j = 0; j < B.numCols(); j += 2) {
      for (size_t k = 0; k < A.numCols(); ++k) {
        C(i, j) += A(i, k) * B(k, j);
        C(i, j + 1) += A(i, k) * B(k, j + 1);
...
void blockedTiledMultiply2x2(const Matrix& A, const Matrix& B, Matrix& C) {
  const size_t blocksize = std::min(A.numRows(), 64UL);

  for (size_t ii = 0; ii < A.numRows(); ii += blocksize) {
    for (size_t jj = 0; jj < B.numCols(); jj += blocksize) {
      for (size_t kk = 0; kk < A.numCols(); kk += blocksize) {
...
void streamMatrix(const Matrix& A, ostream& outputFile) {
  ...
  outputFile << A.numRows() << " " << A.numCols() << endl;
